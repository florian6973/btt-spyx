...
cell = EligALIF(n_in=n_in, n_rec=n_LIF + n_ALIF, tau=tau_v, beta=beta, thr=thr,
                ...
                stop_z_gradients=True,#True,  # This option makes it possible to blindly compute e-prop with auto-diff.
                n_refractory=n_ref)

# Define an arbitrary input, here some poisson spike train.
inputs = tf.random.uniform(shape=[1, T, n_in]) < f0 * dt / 1000
inputs = tf.cast(inputs, tf.float32)

inputs_new = np.load("inputs.npz")['inputs']
inputs_new = tf.constant(inputs_new, dtype=tf.float32)
# inputs = inputs_new
...
# w_out = tf.random.normal(shape=[n_rec, 1])

w_out = np.load("params.npz")['w_out']
...
print(y_out.shape)
# exit()
y_target = tf.random.normal(shape=[1, T, 1])

# y_target = np.load("lsnn2.npz")['y_target']
...
gradients_eprop, eligibility_traces, _, _, v_scaled, psi, ea, ev = \
...
gradients_autodiff2 = tf.gradients(loss, w_out)[0]
gradients_autodiff3 = tf.gradients(loss, cell.w_in_var)[0]
...
tf_tensors = {'inputs': inputs,
              ...
              'gradients_autodiff': gradients_autodiff,
                'gradients_autodiff2': gradients_autodiff2,
                'gradients_autodiff3': gradients_autodiff3,
              ...
              'learning_signals': learning_signals,
                'voltages': voltages,
                'thr_variations': thr_variations,
                'pre_synpatic_spike_one_step_before': pre_synpatic_spike_one_step_before,
                'v_scaled': v_scaled,
...
print(np_tensors['learning_signals'])
print(np_tensors['spikes'])
print(np_tensors['voltages'])
print(np_tensors['thr_variations'])
print(np_tensors['pre_synpatic_spike_one_step_before'])
print(np_tensors['eligibility_traces'])
print(np_tensors['y_target'])
print(np_tensors['y_out'])
print(np_tensors['w_out'])
print(np_tensors['z_filtered'])
print(np_tensors['gradients_autodiff2'])
print(np_tensors['gradients_autodiff3'])
# print(np_tensors['v_scaled'])
# print(np_tensors['psi'])
# print(np_tensors['ea'])
# print(np_tensors['ev'])
# exit()
...
np.savez(
    'data.npz',
    learning_signals=np_tensors['learning_signals'],
    spikes=np_tensors['spikes'],
    voltages=np_tensors['voltages'],
    thr_variations=np_tensors['thr_variations'],
)
